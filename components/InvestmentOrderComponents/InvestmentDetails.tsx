...
import { chargesProp, detailsProp, holdingProp } from "../../model/transaction";
...
const InvestmentDetails = ({ detailsName, value }: dataProps) => {
    ...
    switch (valueClicked) {
      case "Details":
        const detailsData = {
          ...
          "Order Status":
            value?.completeOrderStatus == "FULFILLED"
              ? "Completed"
              : value?.completeOrderStatus == "INITIATED"
              ? "Pending"
              : "Failed",
      ...
      case "Holdings":
        ...
        for (let index = 0; index < value.constituents.length; index++) {
          ...
          holdingData["Avg. Buy Price"].push(
            `${
              value.constituents[index].unitsBought *
              value.constituents[index].weight
            }`
          );
  ...
  return (
    <View style={styles.container}>
      ...
      {details && <InvestmentOrderRowWiseData data={valueForDetails} />}
      {holdings && <HoldingInvestmentData data={valueForHolding} />}
      {charges && <InvestmentOrderRowWiseData data={valueForCharges} />}
    </View>
  );
};
